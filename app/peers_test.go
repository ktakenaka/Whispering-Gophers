...
func TestPeers(t *testing.T) {
	...
	// It may be the solution to run the test fast to use goroutine :thinking:
	go func() {
	...
	go func() {
		for _, ch := range list {
			...
			case <-time.After(10 * time.Millisecond): // うまく使えばパフォーマンスを保ったままみたいなテストにも使えそう
			}
		}
		done <- true
	}()
	...
	case <-done:
		t.Fatalf(`didn't receive message on "a" channel`)
	}
	<-done
	...
	case <-done:
		t.Fatalf(`didn't receive message on "b" channel`)
	}
}